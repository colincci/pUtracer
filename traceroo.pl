...
sub init_logging {
    Log::Log4perl::Logger::create_custom_level( "AUDIT", "WARN" );
    $LogLevel = Log::Log4perl::Level::to_priority($LogLevelStr);    # Set the global loglevel variable.

    if ( defined $$LogConf ) {
        Log::Log4perl->init($LogConf) || die "Unable to open log config: '$LogConf'\n\n";
    }
    else {
        # Define main category logger
        my $log = get_logger("");                                   #Note: "" is key to getting the RootLogger
        my $screen_layout    = Log::Log4perl::Layout::PatternLayout->new("[%p] %M (%L) %m%n");
        my $logfile_layout   = Log::Log4perl::Layout::PatternLayout->new("[%d{yyyyMMdd.HH:mm.ss}] %p %M (%L) %m%n");
        my $auditfile_layout = Log::Log4perl::Layout::PatternLayout->new("%d %m%n");
        my $audit_filter = Log::Log4perl::Filter::LevelMatch->new( LevelToMatch => "AUDIT" );

        my $file_appender = Log::Log4perl::Appender->new(
            "Log::Log4perl::Appender::File",
            name     => "filelog",
            filename => "$LogPath/$ServerName-$DateTime.log"
        );

        my $auditfile_appender = Log::Log4perl::Appender->new(
            "Log::Log4perl::Appender::File",
            name     => "auditfilelog",
            filename => "$LogPath/$ServerName-$DateTime.audit"
        );
        ...
        $file_appender->layout($logfile_layout);
        
        $auditfile_appender->layout($auditfile_layout);
        $auditfile_appender->filter($audit_filter);
        
        $log->add_appender($stdout_appender);
        $log->add_appender($file_appender);
        $log->add_appender($auditfile_appender);
        $log->level($LogLevel);
    }
}
